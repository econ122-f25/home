...
ggplot(results_long, aes(x = Degree, y = Test_MSE, group = Split, color = factor(Split))) +
...
```{r knn-cv-plot}
# Create a plot of CV error vs k
...
ggplot(knn_results, aes(x = k, y = cv_error)) +
  geom_line(color = "steelblue", size = 1.2) +
  geom_point(color = "steelblue", size = 3) +
  geom_vline(xintercept = best_k, color = "red", linetype = "dashed", size = 1) +
  annotate("text", x = best_k + 5, y = min(cv_errors) + 0.01, 
...
```{r comparison-tables}
# Create regression methods comparison
regression_comparison <- data.frame(
  Method = c("Validation Set", 
             "10-fold CV", 
             "LOOCV"),
  `Best_Model_Complexity` = c("", "", ""),  # Fill in (e.g., "Degree 2")
  `Test_MSE` = c("", "", ""),                # Fill in with MSE values
  `Computational_Cost` = c("Low", "Medium", "High"),
  stringsAsFactors = FALSE
)

# Create classification methods comparison
classification_comparison <- data.frame(
  Method = c("Logistic Regression (Linear)", 
             "Logistic Regression (Polynomial)",
             "k-NN LOOCV"),
  `Best_Model_Complexity` = c("", "", ""),   # Fill in (e.g., "Linear terms", "k = 21")
  `Error_Rate` = c("", "", ""),              # Fill in with error rates
  `Accuracy` = c("", "", ""),                # Fill in with accuracy percentages
  `Computational_Cost` = c("Medium", "Medium", "High"),
  stringsAsFactors = FALSE
)

# Print both tables
knitr::kable(regression_comparison, 
             caption = "Regression Methods Comparison (Auto dataset - predicting MPG)")

cat("\n\n")

knitr::kable(classification_comparison, 
             caption = "Classification Methods Comparison (Default dataset - predicting default)")
```
