...
**Hint:** To add multiple predictors in a linear model, use the `+` operator in your formula. For example, if you wanted to predict `y` using both `x1` and `x2`, you would write: `lm(y ~ x1 + x2, data = your_data)`
...
**Hint:** Use `summary(fit_multiple)$coefficients[, "Pr(>|t|)"]` to extract the p-values with more precision
